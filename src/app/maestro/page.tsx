...
export default function MaestroPage() {
  ...
  const generate = () => {
    if (!objetivo || !energia || !canal) return;
    setGenerating(true);

    // Simulate AI generation with smart combinations
    setTimeout(() => {
      ...
      const canalLabel = canales.find(c => c.id === canal)?.label || "Instagram";
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-heading font-semibold text-senda-jet">El Maestro</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Canal */}
          <div className="card">
            <h2 className="font-heading font-semibold text-senda-jet mb-3">¿En qué canal?</h2>
            <div className="flex gap-2">
              {canales.map(c => (
                <button
                  key={c.id}
                  onClick={() => setCanal(c.id)}
                  className={`chip flex-1 justify-center ${canal === c.id ? "chip-active" : "chip-inactive"}`}
                >
                  {c.emoji} {c.label}
                </button>
              ))}
          ...
          <button
            onClick={generate}
            disabled={!objetivo || !energia || !canal || generating}
            className="btn-yellow w-full text-base py-3 disabled:opacity-50"
          >
            {generating ? "Generando..." : "🎯 Generar sugerencia"}
          </button>
        </div>
